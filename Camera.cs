...
namespace MonoUndertale
{
    ...
    public class Camera
    {
        ...
        // bounds of the map.
        public void MoveCamera(Vector2 cameraMovement, Rectangle MapSize, bool clampToMap = false)
        {
            Vector2 newPosition = Position + cameraMovement;

            if (clampToMap)
            {
                Position = MapClampedPosition(newPosition, MapSize);
            }
            else
            {
        ...
        public void CenterOn(Sprite cell, Rectangle MapSize)
        {
            Position = CenteredPosition(cell,  MapSize, true);
        }

        private Vector2 CenteredPosition(Sprite cell, Rectangle MapSize, bool clampToMap = false)
        {
            var cameraPosition = new Vector2(cell.Position.X * cell.Width,
               cell.Position.Y * cell.Height);
            var cameraCenteredOnTilePosition =
               new Vector2(cameraPosition.X + cell.Width / 2,
                   cameraPosition.Y + cell.Height / 2);
            if (clampToMap)
            {
                return MapClampedPosition(cameraCenteredOnTilePosition, MapSize);
            }
        ...
        private Vector2 MapClampedPosition(Vector2 position, Rectangle MapSize)
        {
            var cameraMax = new Vector2(MapSize.Width -
